...
fn processes_single_request_metrics() {
    ...
    assert_eq!(
        D.lock().take_request(),
        DecoderRequest::new(DecoderRequestFlags::METRICS, 0)
    );
}
...
fn processes_single_request_key() {
    ...
    assert_eq!(
        D.lock().take_request(),
        DecoderRequest::new(DecoderRequestFlags::METRICS | DecoderRequestFlags::KEY, 0)
    );
}
...
fn processes_single_track_request() {
    ...
    assert_eq!(
        D.lock().take_request(),
        DecoderRequest::new(DecoderRequestFlags::METRICS, 1)
    );
}
